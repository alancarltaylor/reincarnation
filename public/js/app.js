$(function() {
  ...
  //These are the stories of what happened in between each death and new birth
  ...
  $('button').on('click', function() {
    ...
    //Setting the birthyear to local storage
    localStorage.setItem('bYear', birthDateWithoutMinusOneYear);

    //Check if user enters a birthdate in an expected range
  ...
  function getPerson(deathYear) {
    ...
    //Ajax call, with the user's birthyear minus one, now set as the variable 'deathYear'
    $.ajax({
    ...
    .then(function(response) {
      ...
      //This counter makes sure there are 5 past lives shown
      if (data.counter < 4) {
        data.counter += 1;
  ...
  //This function updates the Dom with the responses from Wikipedia
  function updateDom() {
